...
jQuery.getJSON("data/indonesia.geojson",function(data) {
	uk_map=data;
	addMap();
});
...
$(document).ready(function() {
		...
		map = L.map('map',mapOpts).setView([-2.219622, 116.564668], 4);
		addMap();
		...
		/*geojson = L.geoJson(uk_post_districts, {
			...
			info.update = function (props) {
				if (props) {
					...
					var str = '<h5>Twitter Fans Suporter</h5><p>Untuk provinsi : ' + postcode+"</p>";
					...
					this._div.innerHTML = '<h5>Twitter Fans Suporter </h5><small>Hover mouse ke tiap provinsi</small>';
				}
			};
			info.addTo(map);			
	...
	function addMap() {
		...
		function dataReady() {
			...
			for (var i=0; i<uk_post_districts.features.length; i++){
				var postcode = uk_post_districts.features[i].properties["PROV"];
			...
			function style(feature) {
				...
				var postcode=feature.properties.PROV;
